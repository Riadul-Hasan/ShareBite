...
const SingUp = () => {
  ...
  createUser(email, password)
    ...
    return (
        ...
        <div className="min-h-[calc(100vh-320px)] flex flex-col items-center justify-center px-4 bg-gradient-to-br from-amber-50 via-orange-50 to-rose-100">
    <div className="card bg-white bg-opacity-80 backdrop-blur-sm rounded-2xl w-full max-w-sm shadow-xl mx-auto mt-20 mb-20 border border-orange-100">
        <div className='p-4 rounded-t-2xl bg-gradient-to-r from-amber-500 to-amber-600'>
            ...
            <p className='text-sm text-center text-amber-100'>Join Us Today</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="card-body">
            ...
            <fieldset className="fieldset">
                ...
                <input name='name' required type="text" className="input input-bordered mb-2" placeholder="Name" />
                
                <label className="label font-semibold text-gray-600">Email</label>
                <input name='email' required type="email" className="input input-bordered mb-2" placeholder="Email" />
                
                <label className="label font-semibold text-gray-600">Photo Url</label>
                <input name='photoUrl' required type="text" className="input input-bordered mb-2" placeholder="Photo url" />
                
                <label className="label font-semibold text-gray-600">Password</label>
                <input name='password' required type="password" className="input input-bordered mb-2" placeholder="Password" />
                
                {error && <p className='text-xs font-semibold text-red-600'>{error}</p>}

                <button className="btn bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white mt-4 border-none shadow-md">
                    Create Account
                </button>
            </fieldset>
        </form>

        <div className="divider text-gray-400 text-xs px-4 before:bg-gray-200 after:bg-gray-200">Or Register With</div>

        <div className='text-center mb-4'>
            <button onClick={handleGoogleSignIn} className="btn w-11/12 mx-auto p-2 border bg-white border-gray-200 hover:bg-gray-50 text-gray-700 flex items-center shadow-sm">
                <FcGoogle className="text-xl mr-2"/>
                Sign In with Google
            </button>
        </div>

        <p className='text-center font-semibold pb-4 text-gray-600'>
            Already have an account? 
            <Link to="/login" className='text-amber-600 hover:text-amber-700 font-semibold ml-1'>Login Now</Link>
        </p>
    </div>
</div>
    );
